...
bool solve(vector<long>& a, vector<long>& b, vector<long>& c, long total) {
    bool finds[3] = {false, false, false};
    int n = a.size();
    int j = 0;
    long sum = 0;
    ans[0] = 1;
    ...
    return finds[0] && finds[1] && finds[2];
}
...
void solve() {
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        bool finds = false;
        if (i == 0) finds = solve(aa, bb, cc, total);
        if (i == 1) finds= solve(aa, cc, bb, total);
        if (i == 2) finds = solve(bb, aa, cc, total);
        if (i == 3) finds = solve(bb, cc, aa, total);
        if (i == 4) finds = solve(cc, aa, bb, total);
        if (i == 5) finds = solve(cc, bb, aa, total);
        if (finds) {
            ans[5] = n;
